...
- **Documentation** (`.md .mdx .markdown .rst .adoc .asciidoc`) joins the pass when it is itself changed, or when a changed source file sits in the same directory or beneath it
- **Never** `CLAUDE.md` or `CLAUDE.local.md` — instruction, not documentation of code
- **The rejected-approaches log** (`docs/lessons.md` by default) is judged like any doc, against 2 countermands: the file is never deleted outright, and no entry is removed for being stale or overtaken. Whether a rejection still holds needs a live discussion — `/architect` owns that, this pass judges wording only
