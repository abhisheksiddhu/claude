...
## The rejected-approaches log

The log records the **past** — considered, not done. A rejection restatable as a current-state rule is a README line, not an entry.

### What earns an entry

All 3 hold, or no entry:

1. **Re-proposable** — someone with the same context suggests it again.
2. **Expensive to re-derive** — the rejection cost real investigation. Anything hit within 5 minutes of trying goes unlogged.
3. **Durable reason** — a property of the technology, protocol or domain. Never a snapshot of scale, pricing, volume, team, deadline or library version.

Test 3 does the work. "For now", "at this stage", "currently", or a number that will move → fails it.

    - **{Approach}** — rejected because {reason}. {Exit condition, if any.}

### Override and cleanup

An entry is evidence, not a veto — you hold less context than the user on what has changed since it was written.

On a Phase 3 match: surface it verbatim, then **require an explicit user override**. Your own analysis never suffices. Overridden → delete the entry this session.

At Phase 7, sweep wider: entries this session's decision invalidates that Phase 3 never matched — siblings resting on the same reason, entries whose reason was a snapshot of a stage this decision moves past.

Delete outright, without asking, 1 report line each. Never mark superseded, strike through, or append a correction — git holds the history, and an entry carrying its own contradiction costs the reader more than its absence.

Age ≠ staleness. Delete on the reason no longer holding, judged against the discussion.
...
- Check the rejected-approaches log for entries matching the current direction. Every match runs the override protocol under `## The rejected-approaches log` — surfaced verbatim, overridden only by the user, deleted once overridden
...
- **Rejected approaches** — write and sweep the log per `## The rejected-approaches log`. Append a one-liner only for a rejection clearing all 3 tests there; most do not. Then run the broad sweep: this session's decision can invalidate entries Phase 3 never matched, and those go now
