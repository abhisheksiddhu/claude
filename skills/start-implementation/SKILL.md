...
Stop at the report — a dry loop is not a close signal, so no commit message and no commit suggestion. When the user does signal close (*wrap up*, *ship it*, *commit*), that is the go-ahead itself: run `/prose-prune`, then `/commit`, then stop. Do not ask first. Full rules: **Commit handoff** in the global `CLAUDE.md`.
